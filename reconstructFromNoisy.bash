...
printf "%3.4f\t%40s\n" $ang "${basest}-${Z}.mrc" >> ${basest}/denoised/$listfile 
echo $Z $ang

#Target directory to copy frames from
newline="../SumFrames/denoised/${noSuf}_sumavg.mrc"
#Target location for individual frames corresponding to stack z number
newname="${basest}/denoised/${basest}-${Z}.mrc"
...
rm $home/stacks/$pathfile
cd $home
mv stacks/${basest}/denoised/ AlignedStacks/$basest
rm -r stacks/$basest

#Copy required etomo generated files from /denoised/
cd AlignedStacks/$basest/
mkdir denoised/$basest/
cp $basest/*.xf denoised/$basest/
cp $basest/*.com denoised/$basest/
cp $basest/*.tlt denoised/$basest/
cp $basest/*.xtilt denoised/$basest/
cp denoised/*.st denoised/$basest/
cd denoised/$basest/
